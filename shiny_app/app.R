...
load("sampling_design.rda")
load("one_parameter.rda")
load("multi_parameter.rda")
      ...
      title = "Read Me!", 
        navset_card_underline(
             nav_panel("Introduction",
               ...
               tags$span("Methods in Ecology and Evolution", style = "font-size: 16px; font-style: italic"),
               ...
               tags$span("Occupancy models are frequently used to estimate species distributions while 
                         accounting for false absences in detection-nondetection data. When applying such models 
                         across large spatial extents, it is often recommended to account for residual spatial 
                         autocorrelation within the occupancy model structure using spatial occupancy models. 
                         While such models are seeing increasing adoption by ecologists and practitioners, 
                         there remains little practical guidance on the impacts of survey design on the accuracy 
                         and reliability of predictions from spatial occupancy models. We perform an extensive 
                         simulation study to assess the impacts of survey design and species characteristics on the 
                         accuracy and precision of spatial occupancy model estimates. We explicitly quantify the effects
                         of sample size, sampling design, species prevalence, and effective spatial range of spatial 
                         autocorrelation on the accuracy and precision of occupancy predictions.", style = "font-size: 14px"), 
               tags$span("Key Results:", style = "font-size: 16px; font-weight: bold"),
               tags$span("We found that sample size and sampling design had the most prominent effects on spatial occupancy model performance 
                         with all other tested characteristics having relatively little or no effect. Estimates were 
                         positively biased when the number of sampling locations was low (50, 100), with bias effectively 
                         disappearing with 200 sampling locations. Preferential sampling resulted in positive bias in 
                         occupancy estimates, while all other sampling designs (i.e., simple random sampling, systematic sampling, 
                         and four types of cluster sampling) had minimal bias when sample sizes were large enough, suggesting 
                         practitioners can adopt a probability sampling design most suited to their logistical needs without 
                         sacrificing any statistical efficiency. In comparison to estimates from a nonspatial occupancy model, 
                         spatial occupancy models always had less than or equal bias and substantially more optimal 95% credible 
                         interval coverage rates, suggesting that spatial occupancy models should be used if residual spatial 
                         autocorrelation is present, even when the number of sampling locations is low. Our findings lend strong 
             ...
             nav_panel("App Instructions",
               tags$span("This R Shiny application allows users to explore the results of our simulation study from Doser et al. 2026.",
             ...
             nav_panel("Terminology",
               ...
               tags$span(
                 tags$ol(
                   ...
                   tags$li("Width of 95% credibel intervals - a measure of uncertainty in subsequent model predictions", style = "margin-bottom: 8px")
                 ),
               style = "font-size: 16px"),#end of performance metrics list
               
               tags$span("Model Types", style = "font-size: 22px; font-weight:bold"),
               
               tags$span("We assessed model performance in scenarios where the model was correctly specified, referred to as the 'spatial model' as well
                         as in a mis-specification scenario in which two additional spatially-autocorrelated covariates influenced occupancy probability
                         but were not included in the model, referred to as the 'nonspatial model'.", style = "font-size: 16px")
             ) #end of nave_panel3    
