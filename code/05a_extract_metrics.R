# 05a_extract_metrics.R: script to process the massive amount of results files
...
for (i in 1:n_total) {
  ...
  tryCatch({
    load(Sys.glob(paste0(results_dir, 'occupancy-sampling-row-', curr_row, 
...
save(summary_df, file = paste0(results_dir, "summary_sim_1_results.rda"))
