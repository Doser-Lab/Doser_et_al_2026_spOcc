# 04a_main_nonspatial.R: script to run the complete set of simulations with a nonspatial 
...
for (i in 1:n_iters) {
  ...
  for (l in 1:n_landscapes) {
    ...
    load(paste0(data_dir, "sim_data/landscape_", l, "_simulation_", i, ".rda"))

    output <- run_simulation(dat = dat, n.plots = n.plots, n.neighbors=neighbors, method=design, 
                             nonspatial = TRUE)
   
    # Save the output. 
    save(output, file=paste0(results_dir, 'nonspatial-occupancy-sampling-row-', curr_row, '-landscape-', l, 
