#!/usr/bin/bash
#
# Script:  03_submit.sh
# Usage: For submitting multiple batch jobs to the HPC.
# Author: 
#
# To run, type:
#     ./03_submit.sh [start_row end_row replicates] 
#  Script must have execute permissions, i.e.,
#     chmod u+x 03_submit.sh

module load openmpi-gcc
module load R

# Some quick checks
if [ $# -ne 3 ]; then
        echo "Usage: You need to feed three arguments to this program which are"
        echo "the start and end row of the parameters, and the number of replicates. For example,"
        echo "./occ_research_submit.sh 1 40 10"
        exit 1
fi


#Read in variables
start_row=$1
end_row=$2
reps=$3


# Output printed to the screen
while [ $start_row -le $end_row ]
do 

  echo "Submit job = row:$start_row, replicates:$reps"
  
  bsub -n 1 -W 7200 -R span[hosts=1] -R "rusage[mem=5]" -q cnr -oo out.03_main_test.$start_row -eo err.03_main_test.$start_row "Rscript 03_main_misspec.R $start_row $reps"

  ((start_row++))

done
